...
public class Grid{
    ...
    if (size == 1) {
        ...
        return new Node(getNode(x, y).isAlive());  // Leaf node (single cell)
    }
    
    System.out.println("toQuadtree size " + size);
    int halfSize = size / 2;

    // Recursively create the four quadrants
    ...
    // Create an internal node combining the four quadrants
   ...
   public void fromQuadtree(Node node, int x, int y, int size) {
    if (node.isLeaf()) {
        // If it's a leaf node, set the grid cells' state based on the node's 'alive' state
        System.out.println("fromQuadtree size " + size);
        
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                System.out.println("fromQuadtree: x=" + (x + j) + ", y=" + (y + i) + ", alive=" + node.isAlive());
    ...
    // Recursive calls for sub-quadrants
    int halfSize = size / 2;
